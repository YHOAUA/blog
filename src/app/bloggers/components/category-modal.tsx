...
interface BloggerCategoryModalProps {
	open: boolean
	onClose: () => void
	categoryList: string[]
	newCategory: string
	...
	bloggers: Blogger[]
	onAssignCategory: (url: string, category?: string) => void
}

export function BloggerCategoryModal({
	open,
	onClose,
	categoryList,
	newCategory,
	...
	return (
		<DialogModal open={open} onClose={onClose} className='card w-[720px] max-w-[90vw] rounded-2xl p-6'>
			<div className='mb-4 flex items-center justify-between'>
				<div className='text-lg font-semibold'>博主分类</div>
			...
			<div className='space-y-4'>
				...
				<div className='max-h-[360px] space-y-2 overflow-y-auto rounded-xl bg-white/60 p-3'>
					{bloggers.map(blogger => (
						<div key={blogger.url} className='flex flex-col gap-2 rounded-lg border bg-white/80 px-3 py-2 sm:flex-row sm:items-center sm:justify-between'>
							<div className='flex items-center gap-2 text-sm font-medium'>
								<img src={blogger.avatar} alt='' className='h-6 w-6 rounded-full object-cover' />
								<span className='truncate'>{blogger.name}</span>
							</div>
							<Select
								value={blogger.category || ''}
								onChange={value => onAssignCategory(blogger.url, value)}
								options={categoryOptions}
								className='w-full text-sm sm:w-[180px]'
							/>
						</div>
					))}
					{bloggers.length === 0 && <div className='text-secondary text-sm'>暂无博主</div>}
				</div>
			</div>
		</DialogModal>
	)
